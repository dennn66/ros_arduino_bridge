...
from ros_arduino_python.arduino_ip_driver import ArduinoIP
from ros_arduino_python.arduino_sensors import *
from ros_arduino_msgs.srv import *
from ros_arduino_python.base_controller import BaseController
from ros_arduino_python.uarm_controller import UarmController
from ros_arduino_python.camera_controller import CameraController
...
class ArduinoROS():
    def __init__(self):
        ...
        self.use_uarm_controller = rospy.get_param("~use_uarm_controller", False)
        self.use_camera_controller = rospy.get_param("~use_camera_controller", False)
        ...
        self.controller = ArduinoIP(self.port, self.baud, self.timeout)
        ...
        for name, params in sensor_params.iteritems():
            ...
            elif params['type'] == 'CustomMotorCurrent':
                sensor = CustomMotorCurrent(self.controller, name, params['pin'], params['rate'])
            elif params['type'] == 'CustomVoltage':
                sensor = CustomVoltage(self.controller, name, params['pin'], params['rate'])
            elif params['type'] == 'CustomCurrent':
                sensor = CustomCurrent(self.controller, name, params['pin'], params['rate'])
...
#                    self.sensors[len(self.sensors)]['output_pin'] = params['output_pin']
        ...
        # Initialize the uarm controller if used
        if self.use_uarm_controller:
            self.myUarmController = UarmController(self.controller)

        # Initialize the camera controller if used
        if self.use_camera_controller:
            self.myCameraController = CameraController(self.controller)
        ...
        while not rospy.is_shutdown():
            ...
            if self.use_uarm_controller:
                mutex.acquire()
                self.myUarmController.poll()
                mutex.release()
            
            if self.use_camera_controller:
                mutex.acquire()
                self.myCameraController.poll()
                mutex.release()
