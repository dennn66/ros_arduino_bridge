...
class ArduinoOverIP:
    ...
    def __init__(self, host="192.168.1.120", port=2000, timeout=0.5):
    ...
    def connect(self):
        try:
            print "Connecting to Arduino on host", self.host, ":", self.port, "..."
            #self.port = Serial(port=self.port, baudrate=self.baudrate, timeout=self.timeout, writeTimeout=self.writeTimeout)
            self.tn = telnetlib.Telnet(self.host, self.port, self.timeout)
	    self.tn.write("hello world\r\n")
    ...
    def recv(self, timeout=0.5):
        ...
        #        return None
	value = self.tn.read_until('\r', timeout)
        value = value.strip('\r')
...
if __name__ == "__main__":
    ...
    myArduino = ArduinoOverIP(host="192.168.1.120", port=2000, timeout=0.5)
    myArduino.connect()
